...
"""))
    ...
    def send_captcha(self, new_session = False):
        ...
        if self.get_captcha(): print(Colorate.Horizontal(Colors.red_to_blue, f' Đang Kêt Nối Đến Session Thinhdepzai'));return (True, 'The session already exists')
        ...
        if 'Enter Video URL' in request.text: 
            print(Colorate.Horizontal(Colors.green_to_white, f' Session Thinhdepzai Đã Được Tạo'))
...
if __name__ == "__main__":
    print(Colorate.Horizontal(Colors.red_to_blue, TmrVirus_RVO))
    TmrVirusDz = TmrVirus()
    print(Colorate.Horizontal(Colors.red_to_blue, TmrVirus_RVO))
    TmrVirusDz.info_video()
    threading.Thread(target=TmrVirusDz.check_config).start()
    threading.Thread(target=TmrVirusDz.update_name).start()
    TmrVirusDz.send_captcha()
    TmrVirusDz.run()